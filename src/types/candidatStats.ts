 ...
 * 3) Fallback lisible.
 */
export function resolveCandidatGroupLabel(
  row: CandidatGroupRow,
  by: CandidatGroupBy
): string {
  if (row.group_label && String(row.group_label).trim() !== "") return String(row.group_label);

  if (by === "centre" && row["formation__centre__nom"]) return String(row["formation__centre__nom"]);
  ...
  if (by === "formation")
    return row.formation_id != null ? `Formation #${row.formation_id}` : "—";
  if (by === "departement") return row.departement ?? "—";
  if (by === "entreprise")
    return row.entreprise_placement_id != null
...
export async function getCandidatOverview(filters: CandidatFilters) {
  ...
  const { data } = await api.get<CandidatOverviewResponse>("/candidat-stats/", { params });
  return data;
}
